...
describe('dmsToDecimal', () => {
    ...
    it('rejects invalid minutes range', () => {
        ...
        expect(() => baseFormat.dmsToDecimal({ degrees: 45, minutes: 60, seconds: 34, direction: 'W' })).toThrowError(
            `Schema validation failed for parameter 'dms'. Expected to match schema 'union'. [minutes]: Too big: expected number to be <60. Received: {"degrees":45,"minutes":60,"seconds":34,"direction":"W"}`
        );
    });
    ...
    it('rejects invalid seconds range', () => {
        ...
        expect(() => baseFormat.dmsToDecimal({ degrees: 45, minutes: 56, seconds: 60, direction: 'W' })).toThrowError(
            `Schema validation failed for parameter 'dms'. Expected to match schema 'union'. [seconds]: Too big: expected number to be <60. Received: {"degrees":45,"minutes":56,"seconds":60,"direction":"W"}`
        );
    });
});
